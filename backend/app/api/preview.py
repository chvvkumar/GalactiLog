...
async def get_preview(
    ...
    if cache.has(cache_key) and cached_path.exists():
        ...
        return _redirect_response(cache_key)
    ...
    temp_path = previews_dir / f".{cache_key}.tmp"
    try:
    ...
    cache.ensure_capacity_for(size)
...
def _redirect_response(cache_key: str) -> Response:
    resp = Response(status_code=200)
    resp.headers["X-Accel-Redirect"] = f"/_previews_internal/{cache_key}"
